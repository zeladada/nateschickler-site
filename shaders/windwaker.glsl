...
vec3 water(vec2 uv, vec3 cdir) {
    ...
    d2 = time * 0.9 + d2;
    vec2 dist = vec2(
    	sin(d1) * 0.15 + sin(d2) * 0.1,
    	cos(d1) * 0.15 + cos(d2) * 0.1
    );
...
vec3 pixtoray(vec2 uv)
{
    vec3 pixpos;
    pixpos.xy = uv - 0.75;
    pixpos.y *= resolution.y/resolution.x; // Aspect correction
...
void main(void) {
    ...
    for(int y = 0; y < 2; y++) {
        for(int x = 0; x < 2; x++) {
            vec2 offset = vec2(0.5, 0.5) * vec2(x, y) - vec2(0.25, 0.25);
