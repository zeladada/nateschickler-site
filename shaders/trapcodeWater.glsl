...
float fnoise( vec3 p)
{
    mat3 rot = rotationMatrix( normalize(vec3(0.0,0.0, 1.0)), 0.5*time);
...
float getHeight(vec3 p) // x,z,time
{
 	...
 	return 0.3-0.5*fnoise( vec3(0.5*(p.x + 0.0*time), 0.5*p.z,  0.4*time) );
}
...
void main( void )
{
    ...
    gl_FragColor = trace_heightfield( ro, rd );
}